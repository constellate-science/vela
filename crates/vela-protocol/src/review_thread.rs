...
// ---------------------------------------------------------------------------
// On-disk storage (merged from the former `review_thread_storage` module,
// v0.174). Threads persist under `<frontier>/.vela/review-threads/`, one JSON
// file per content-addressed `vrt_*` id. Pure filesystem helpers; the
// workbench read surface and the v0.168 CLI commands compose against these.
// ---------------------------------------------------------------------------

/// Resolve the threads directory for a frontier path. Accepts either a
/// frontier-dir path or a `frontier.json` file path.
...
/// malformed files quietly so a single bad file doesn't block the rest of
/// the surface. Returns threads sorted by `created_at` ascending.
...
/// Load one thread by `vrt_*` id. Returns None if the file is missing or
/// unparsable.
...
/// Persist a thread under `<frontier>/.vela/review-threads/`. Creates the
/// directory if needed.
...
mod storage_tests {
    ...
    fn load_all_skips_garbage_files() {
        ...
        .unwrap();
        std::fs::write(dir.join("garbage.json"), "{not valid").unwrap();
