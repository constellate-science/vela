...
pub(crate) use crate::cli_export::*;
    ...
    match Cli::parse().command {
        ...
        Commands::Onboard { frontier, json } => {
            ...
            for f in &project.findings {
                ...
                if f.flags.gap && t.contains("OBLIGATION") {
                    if let Some(b) = t.find("BANKED:") {
                        let end = t.find("OPEN:").unwrap_or(t.len());
                        banked.push(format!("- {} ({})", t[b + 7..end].trim(), f.id));
                    }
                    if let Some(o) = t.find("OPEN:") {
                        open.push(format!("- {} ({})", t[o + 5..].trim(), f.id));
                    }
                }
            }
            let body = format!(
                "# {name} — agent onboarding (generated by `vela onboard`; do not edit)\n\nThe frontier record is the source of truth. Your output is a state\ntransition or it is nothing: a witness that passes `vela reproduce`,\na finding proposed through the Frontier PR flow, a signed attempt\n(failures included), or a precise obstruction. Strategy prose does\nnot move the frontier.\n\n**Done means `vela gate` passes** (`vela check {dir}` replay-verified\n+ `vela reproduce {dir}` all witnesses green).\n\n## Entry contract\nPull a task packet before working: `vela task packet {dir} <target>`\n(or the `task_packet` MCP tool). It carries the obligation, the pinned\nroot, the minimal kernel-premise slice, allowed outputs mapped to\nverifiers, leases, and failed routes.\n\n## Do not re-grind (banked routes)\n{banked}\n\n## Open targets\n{open}\n\n## Rules\n- An agent may propose; only a key-holding human accepts.\n- Lease before long work: `vela claim {dir} <obligation-id> --ttl 86400 …`.\n- Park scratch artifacts with `vela stash`; reference vsx_ hashes.\n- Deposit a signed attempt even when you fail — especially when you fail.\n",
                name = name,
                dir = frontier.display(),
                banked = if banked.is_empty() {
                    "(none)".to_string()
                } else {
                    banked.join(
                        "
...
",
            ...
            std::fs::write(frontier.join("AGENTS.md"), &body)
                .unwrap_or_else(|e| fail_return(&format!("write AGENTS.md: {e}")));
            std::fs::write(frontier.join("CLAUDE.md"), &body)
                .unwrap_or_else(|e| fail_return(&format!("write CLAUDE.md: {e}")));
            let payload = json!({"ok": true, "command": "onboard", "frontier": frontier.display().to_string(), "banked": banked.len(), "open": open.len()});
            if json {
                print_json(&payload);
            } else {
                println!(
                    "{} wrote AGENTS.md + CLAUDE.md ({} banked routes, {} open targets)",
                    style::ok("ok"),
                    banked.len(),
                    open.len()
                );
        ...
        Commands::Recommend {
            ...
            reason,
            json,
        } => cmd_recommend(frontier, proposal_id, by, key, reason, json),
...
pub(crate) fn print_stats_from_shards_human(path: &Path) -> bool {
