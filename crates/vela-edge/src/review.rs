...
use vela_protocol::bundle::ReviewEvent;
...
pub fn import_review_events(source: &Path, target: &Path) -> Result<ReviewImportReport, String> {
    ...
    vela_protocol::project::recompute_stats(&mut frontier);
...
mod tests {
    use super::*;
    use vela_protocol::bundle::ReviewAction;
    ...
    fn import_review_events_from_json_file_merges_new_event() {
        ...
        let frontier = vela_protocol::project::assemble("target", vec![], 0, 0, "target");
        ...
        let loaded = vela_protocol::repo::load_from_path(&target).unwrap();
